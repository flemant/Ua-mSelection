...
if isempty(FuMeas)
    ...
    fprintf('Loading interpolant for surface velocity data: %-s ',UserVar.SurfaceVelocityInterpolant)
    load(UserVar.SurfaceVelocityInterpolant,'FuMeas','FvMeas','FerrMeas')
    fprintf(' done.\n')
end

% Get measurements from synthetic data
Meas.us=double(FuMeas);
Meas.vs=double(FvMeas);
Err=double(FerrMeas);

%% USE THIS IF USING REAL DATA AND NOT SYNTHETIC DATA
