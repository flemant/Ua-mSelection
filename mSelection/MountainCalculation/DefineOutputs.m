...
CtrlVar.DefineOutputs='-sbB-save-';
...
%% DO NOT CHANGE THIS SECTION; THE DATA FROM THE DIAGNOSTIC RUN ARE SAVED AND THE NAME OF THE FILE SAVED FOR LATER USE

if contains(plots,'-save-')
    
    %save data if forward run
    
   if strcmp(CtrlVar.DefineOutputsInfostring,'Last call')==0
        switch UserVar.RunType
            case 'Forward-Diagnostic'
                FileName=sprintf('FR-%s-m%i',...
                UserVar.RunType,...
                UserVar.m);
                fprintf(' Saving data in %s \n',FileName)
                save(FileName,'CtrlVar','UserVar','MUA','F')
                DiagnosticFilename=FileName+".mat";
                save(UserVar.UserFileName,"m","DiagnosticFilename"); % to get the diagnostic file name        
        end
    end
end
